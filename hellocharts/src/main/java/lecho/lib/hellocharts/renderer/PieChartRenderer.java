    ...
    private void drawCenterCircle(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(data.getCenterText1())) {
            ...
            if (!TextUtils.isEmpty(data.getCenterText2())) {
                ...
                canvas.drawText(data.getCenterText1(),
                        centerX, centerY + text1Height / 4, centerCircleText1Paint);
            }
        }
    }
    ...
    public void drawLabels(Canvas canvas) {
        ...
        for (SliceValue sliceValue : data.getValues()) {
            ...
            if (isTouched()) {
                ...
                if(hasLabelsOutside){
                    drawOutSideLabelDate(canvas, sliceValue, lastAngle, angle);
                    drawOutSideLabelRemark(canvas, sliceValue, lastAngle, angle);
                }
            }
            lastAngle += angle;
            ++sliceIndex;
...
//        }
    ...
    private void drawOutSideLabelDate(Canvas canvas, SliceValue sliceValue, float lastAngle, float angle) {
        sliceVector.set((float) (Math.cos(Math.toRadians(lastAngle + angle / 2))),
                (float) (Math.sin(Math.toRadians(lastAngle + angle / 2))));
        normalizeVector(sliceVector);

        final int numChars = valueFormatter.formatChartValueDate(labelBuffer, sliceValue);
    ...
    private void drawOutSideLabelRemark(Canvas canvas, SliceValue sliceValue, float lastAngle, float angle) {
        sliceVector.set((float) (Math.cos(Math.toRadians(lastAngle + angle / 2))),
                (float) (Math.sin(Math.toRadians(lastAngle + angle / 2))));
        normalizeVector(sliceVector);

        final int numChars = valueFormatter.formatChartValueRemark(labelBuffer, sliceValue);
        ...
        if (rawY > centerY) {
            // Lower half.
            top = rawY +labelHeight*3+ labelMargin;
            bottom = rawY + labelHeight*4 + labelMargin * 3;
        } else {
            top = rawY - labelHeight*4 - labelMargin * 3;
            bottom = rawY - labelMargin*4;
        }
